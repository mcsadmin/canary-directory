...
- The graph database architectural decision (relational only, hybrid, or pure graph) is a **hard prerequisite** for the main build — it must be confirmed with Paul and Yanny before any main build coding session begins. If this has not been confirmed by the end of the canary day, flag it immediately as a blocker, not merely a course-correction condition. Do not allow the main build to begin without it.
...
*Alignment check completed by the Manus AI design task, 13 March 2026. VSM framing reinstated; Paul and Yanny graph database escalation tightened to hard prerequisite.*
