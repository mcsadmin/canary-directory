...
The experiment week uses **three separate Git repositories** — one per project. The meta-canary and project canary repositories are explicitly throwaway; their commit histories do not carry forward. The sandbox repository is the long-term artefact.

| Repository | Domain | Purpose |
|---|---|---|
| `meta-canary-qr` | `meta-canary.commoner.services` | Method rehearsal (QR code generator) — discarded after meta-canary |
| `canary-directory` | `canary.commoner.services` | Directory / verified nodes component — discarded after canary |
| `sandbox-localloop` | `sandbox.localloop-merseyside.co.uk` | The main Local Loop sandbox — the long-term artefact |

The folder structure below applies to **all three repositories**. Each repository is initialised independently. This document defines the agreed folder structure, the ownership model, and the initialisation steps to be completed during the meta-canary.

The meta-canary repository (`meta-canary-qr`) is initialised first — as part of the meta-canary itself. The canary and sandbox repositories are initialised at the start of their respective phases.
...
```
{project-name}/              # e.g. meta-canary-qr, canary-directory, sandbox-localloop
│
├── /code                    # The project build
...
Each repository has its own deployment domain. The three domains are confirmed as follows:

| Repository | Domain | Purpose |
|---|---|---|
| `meta-canary-qr` | `meta-canary.commoner.services` | Method rehearsal (QR code generator) |
| `canary-directory` | `canary.commoner.services` | Directory / verified nodes component |
| `sandbox-localloop` | `sandbox.localloop-merseyside.co.uk` | Full Local Loop sandbox |
...
Create a separate repository for each project. During the meta-canary, initialise `meta-canary-qr`. During the canary phase, initialise `canary-directory`. During the main build, initialise `sandbox-localloop`.

```bash
git init {project-name}      # e.g. meta-canary-qr
cd {project-name}
```
...
GitBook does not automatically discover files in the repository — it only displays files that are explicitly listed in `SUMMARY.md`. Any file committed to the repository but absent from `SUMMARY.md` is **invisible in GitBook**, even though it exists in GitHub.

**Every file committed to the repository must have a corresponding entry added to `SUMMARY.md` in the same commit.** The format is:

```
* [Human-readable title](path/to/file.md)
```

This applies to all new files: session charters, handoff notes, daily state documents, rolling plans, specification briefs, artefacts, and prompt revisions. The coaching agent is responsible for including the `SUMMARY.md` update in every commit that adds a new file. This is not optional and must not be deferred to a later commit.

*Note: this rule was discovered during the meta-canary (13 March 2026), when 20+ files were committed without `SUMMARY.md` entries and became invisible in GitBook. The fix was applied in commit `f2be565`.*
