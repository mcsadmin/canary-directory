...
**The handoff note feeds the method log bucket.** The coaching agent commits the structured handoff note to `/log/` immediately after receiving it. This is the primary mechanism by which warm conversation decisions enter the log and become available to the end-of-day batch.
...
The coaching agent runs six tasks in sequence:

1. **Method log processing** — tags and synthesises all new `/log` entries since the last batch
2. **Daily state document** — artefacts produced, MoSCoW status, rolling plan for the next two days
3. **Anti-loop check** — *backward-looking*: has the build actually progressed today? Is the ratio of planning to building appropriate?
4. **Blind spot check** — *forward-looking*: what has not been discussed that probably should have been?
5. **Gap analysis** — *backward-looking across the whole project*: what was flagged as important in earlier sessions or documents but has not yet been addressed?
6. **Health check** — coaching agent self-assessment: are its outputs useful and specific, or becoming generic?
...
**The coaching agent is responsible for including the `SUMMARY.md` update in every commit that adds a new file.** This is not optional and must not be deferred to a later commit.
